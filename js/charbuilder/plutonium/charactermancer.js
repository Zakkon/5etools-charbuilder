...
class ActorCharactermancerClass extends ActorCharactermancerBaseComponent {
    ...
    async render() {
        ...
        //ADD CLASS BUTTON
        const addClassBtn = $(`<button class="btn ve-btn-default btn-5et btn-sm">Add Another Class</button>`)
        .click(() => {
    ...
    async _class_renderClass(parentDiv_left, parentDiv_right, ix) {
        ...
        const {
            ...
            const cls = this._getClass({ix: clsIx });
            if (!cls) {
            ...
            fnGetAdditionalStyleClasses: classIx => {
                ...
                const cls = this._getClass({ix: classIx});
                if (!cls) { return; }
        ...
        const filterBtn = $("<button class=\"btn btn-xs ve-btn-default btn-5et h-100 btr-0 bbr-0 pr-2\" title=\"Filter for Class and Subclass\"><span class=\"glyphicon glyphicon-filter\"></span> Filter</button>")
        .click(async () => {
            const cls = this._getClass({propIxClass: propIxClass });
            const subcls = this._getSubclass({cls: cls, propIxSubclass: propIxSubclass});
        ...
        if (!this._existingClassMetas.length || !SETTINGS.LOCK_EXISTING_CHOICES) {
            primaryBtn = $("<button class=\"btn ve-btn-default btn-5et btn-xs mr-2\"></button>").click(() => this._state.class_ixPrimaryClass = ix);
           ...
           removeClassBtn = $("<button class=\"btn ve-btn-default btn-5et btn-xs mr-2\"></button>").click(() => {console.log("Remove class " + ix);
        ...
        const minimizerToggle = $("<div class=\"py-1 clickable ve-muted\">[‒]</div>").click(() => {
            ...
            const cls = this._getClass({'propIxClass': propIxClass});
            const subcls = this._getSubclass({
                'cls': cls,
                'propIxSubclass': propIxSubclass
            });
    ...
    async _doHandleExistingClassItems(classes){
        ...
        this._existingClassMetas = classes.map(cls => {
            const _clsIx = this._getExistingClassIndex(cls);
            const _scIx = this._getExistingSubclassIndex(_clsIx, cls.subclass);
     ...
     */
    _getExistingSubclassIndex(classIx, subclass) {
     ...
     */
    _getClass({ix: ix, propIxClass: propIxClass}) {
     ...
     */
    _getSubclass({ cls: cls, propIxSubclass: propIxSubclass, ix: ix })
    {
    ...
    _class_renderClass_stgSelectSubclass({
      ...
      doApplyFilterToSelSubclass: doApplyFilterToSelSubclass
        ...
        if (cls && cls.subclasses && cls.subclasses.length) {
            ...
            isAllowNull: true,
            fnDisplay: ix => {
                const subcls = this._getSubclass({cls: cls, ix: ix });
                if (!subcls) {
    ...
    async _class_renderClass_pStgLevelSelect({
        ...
        if (cls) {
            ...
            const isForceSelect = true; //!existingClassMeta || (this._getExistingClassTotalLevels() === 0 && SETTINGS.LOCK_EXISTING_CHOICES);
            ...
            const e_onChangeLevelSelected = async () => {
                const subclass = this._getSubclass({cls: cls, propIxSubclass: propIxSubclass});
    ...
    */
    ...
    wipeClassState(ix){
        ...
        for(let i = 0; i <= this._state.class_ixMax; ++i){
            ...
            const cls = this._getClass({propIxClass:clsInfo.propIxClass});
            console.log(cls);
        }
    ...
    async getChoiceData(){
        ...
        let out = {};
        out.classes = [];
        out.ixPrimaryClass = state.class_ixPrimaryClass;
        out.numClasses = state.class_ixMax+1;
        out.totalLevels = state.class_totalLevels;
        for(let i = 0; i < out.numClasses; ++i){
            const part = "class_" + i + "_";
            let cls = {};
            cls.ixClass = state[`${part}ixClass`];
            if(cls.ixClass==null){continue;}
            cls.curLevel = state[`${part}curLevel`];
    ...
    _render_$getRows(ix, spells, {isExpandedMatch}) {
        ...
        return Object.entries(byLevel).sort(([kA],[kB])=>SortUtil.ascSort(Number(kA), Number(kB))).map(([,flats])=>{
            ...
            flatsFilterChoose.forEach(flat=>{
                ...
                const $btnFilter = $(`<button class="btn ve-btn-default btn-xxs mx-1" title="Choose a Spell"><span class="fas fa-fw fa-search"></span></button>`).click(async()=>{
    ...
    render($wrp) {
        ...
        $$`<div class="ve-flex-col min-h-0">
			<div class="ve-flex-v-stretch input-group mb-1 no-shrink">
				<label class="btn ve-btn-default btn-5et col-1 px-1 ve-flex-vh-center">${$cbAll}</label>
				<button class="btn-5et ve-btn-default col-1-5">Level</button>
				<button class="btn-5et ve-btn-default col-9-5">Features</button>
			</div>
        ...
        const e_recountASI = () => {
            ...
            for (let ix = 0; ix < this._parent.compClass.state.class_ixMax + 1; ++ix) {
                ...
                const { propIxClass: propIxClass, propCntAsi: propCntAsi } =
                    ActorCharactermancerBaseComponent.class_getProps(ix);
                //Get the class
                const cls = this._parent.compClass._getClass({ propIxClass: propIxClass });
                if (!cls) { continue; }
...
class StatGenUi extends BaseComponent {
    ...
    _render_isLevelOne($wrpTab) {
        ...
        const $btnResetRolledOrArrayOrManual = $(`<button class="btn ve-btn-default btn-xxs relative statgen-shared__btn-reset" title="Reset"><span class="glyphicon glyphicon-refresh"></span></button>`).click(()=>this._doReset());
    ...
    _render_$getStgRolledHeader() {
        ...
        const $btnRandom = $(`<button class="btn btn-xs ve-btn-default mt-2">Randomly Assign</button>`).hideVe().click(()=>{
    ...
    _render_$getStgArrayHeader() {
        const $btnRandom = $(`<button class="btn btn-xs ve-btn-default">Randomly Assign</button>`).click(()=>{
            const abs = [...Parser.ABIL_ABVS].shuffle();
            abs.forEach((ab,i)=>{
                const {propAbilSelectedScoreIx} = this.constructor._array_getProps(ab);
    ...
    _render_$getStgPbHeader() {
        ...
        const $btnReset = $(`<button class="btn ve-btn-default">Reset</button>`).click(()=>this._doReset());

        const $btnRandom = $(`<button class="btn ve-btn-default">Random</button>`).click(()=>{
            this._doReset();
    ...
    _render_$getStgPbCustom() {
        const $btnAddLower = $(`<button class="btn ve-btn-default btn-xs">Add Lower Score</button>`).click(()=>{
        ...
        const $btnAddHigher = $(`<button class="btn ve-btn-default btn-xs">Add Higher Score</button>`).click(()=>{
        ...
        const $btnResetRules = $(`<button class="btn ve-btn-danger btn-xs mr-2">Reset</button>`).click(()=>{
        ...
        const $btnContext = $(`<button class="btn ve-btn-default btn-xs" title="Menu"><span class="glyphicon glyphicon-option-vertical"></span></button>`).click(evt=>ContextUtil.pOpenMenu(evt, menuCustom));
    ...
    static _RenderLevelOneEntity = class {
        ...
        render() {
            ...
            const $btnFilterForEntity = $(`<button class="btn btn-xs ve-btn-default br-0 pr-2" title="Filter for ${this._title}"><span class="glyphicon glyphicon-filter"></span> Filter</button>`).click(async()=>{
            ...
            const $btnPreview = ComponentUiUtil.$getBtnBool(this._parent, this._propIsPreview, {
                html: `<button class="btn btn-xs ve-btn-default" title="Toggle ${this._title} Preview"><span class="glyphicon glyphicon-eye-open"></span></button>`,
            }, );
    ...
    static _RenderLevelOneRace = class extends this._RenderLevelOneEntity {
        ...
        _$getPtsTashas() {
            const $btnToggleTashasPin = ComponentUiUtil.$getBtnBool(this._parent, "common_isShowTashasRules", {
                html: `<button class="btn btn-xxs ve-btn-default ve-small p-0 statgen-shared__btn-toggle-tashas-rules ve-flex-vh-center" title="Toggle &quot;Customizing Your Origin&quot; Section">
...
StatGenUi.CompAsi = class extends BaseComponent {
        ...
        const hk = ()=>{
            ...
            for (; ix < this._parent.state[propCnt]; ++ix) {
                ...
                if (!this._metasAsi[namespace][ix_]) {
                    ...
                    const $btnAsi = namespace !== "ability" ? null : $(`<button class="btn btn-xs ve-btn-default w-50p">ASI</button>`).click(()=>{
                        this._parent.state[propMode] = "asi";
                        this._doPulseThrottled();
                    }
                    );

                    const $btnFeat = namespace !== "ability" ? $(`<div class="w-100p ve-text-center">Feat</div>`) : $(`<button class="btn btn-xs ve-btn-default w-50p">Feat</button>`).click(()=>{
    ...
    _render_getMetaFeat({featStatic=null, propIxFeat=null, propIxFeatAbility, propFeatAbilityChooseFrom}) {
        ...
        const $btnChooseFeat = featStatic ? null : $(`<button class="btn btn-xxs ve-btn-default mr-2" title="Choose a Feat"><span class="glyphicon glyphicon-search"></span></button>`).click(async()=>{
...
StatGenUi.RenderableCollectionPbRules = class extends RenderableCollectionGenericRows {
    ...
    getNewRender(rule, i) {
        ...
        const $btnDelete = $(`<button class="btn btn-xxs ve-btn-danger" title="Delete"><span class="glyphicon glyphicon-trash"></span></button>`).click(()=>{
    ...
    render($wrp) {
        const $wrpsProp = Charactermancer_Background_Characteristics._PROP_METAS.map((propMeta,ixPt)=>{
            ...
            if (this._tables[propMeta.prop]) {
                const $btnToggleMode = $(`<button class="btn ve-btn-default btn-xs" title="Show/Hide Table">View Table</button>`)
                ...
                const $rendered = Vetools.withCustomDiceRenderingPatch(()=>{ //fn
                    ...
                    $rendered.find(`[data-plut-temp-dice]`).each((i,e)=>{
                        ...
                        const $btnsRoller = [...new Array(count)].map((_,i)=>{
                            return $(`<button class="btn btn-xs ve-btn-default">Roll${count > 1 ? ` ${Parser.getOrdinalForm(i + 1)}` : ""}</button>`).click(async()=>{
        ...
        if (hasAnyTable) {
            $btnToggleAllTables = $(`<button class="btn ve-btn-default btn-xs">Hide Tables</button>`).click(()=>{
...
class Charactermancer_Background_Features extends BaseComponent {
    ...
    render($wrp) {
        ...
        const $btnAddManual = $(`<button class="btn ve-btn-default btn-xs btn-5et ml-1"><span class="glyphicon glyphicon-plus"></span> Add Feature</button>`).click(()=>{
    ...
    _render_other() {
        const $btnSelect = $(`<button class="btn ve-btn-default btn-5et w-100 mr-2 mb-2">Choose Background</button>`).click(async()=>{
        ...
        const hkManualMetas = ()=>{
            this._renderCollection({
                ...
                fnGetNew: featureMeta=>{
                    ...
                    const $btnDelete = $(`<button class="btn btn-5et btn-xs ve-btn-danger ml-1" title="Delete"><span class="glyphicon glyphicon-trash"></span></button>`).click(()=>this._state.manualEntryMetas = this._state.manualEntryMetas.filter(it=>it !== featureMeta));
 ...
 */
    ...
    activateListeners($html) {
        ...
        if (this._cntModesAvailable === 2) {
            ...
            this._$btnShowTabStandard = $(`<button class="btn ve-btn-default w-50 btn-5et">${this._compCurrency.rollableExpressionGold ? `Standard Starting Equipment` : `Starting Equipment`}</button>`).click(()=>{
                this._mode = "default";
                this._doHandleModeSwitch();
            }
            );

            this._$btnShowTabGoldAlternative = $(`<button class="btn ve-btn-default w-50 btn-5et">${this._compCurrency.rollableExpressionGold ? `Gold Alternative/Shop` : `Shop`}</button>`).click(()=>{
                this._mode = "gold";
                this._doHandleModeSwitch();
            }
            );
    ...
    _activateListeners_renderTab($html, comp) {
        comp.pRender($html).then($wrpTabInner=>{
            const $btnAccept = $(`<button class="btn ve-btn-default btn-5et ${this._isStandalone ? "mr-3" : "mr-2"}">Confirm</button>`).click(async()=>{
                const activeComps = [this._compDefault, this._compGold].filter(it=>it.isAvailable);
            ...
            const $btnSkip = this._isStandalone ? null : $(`<button class="btn ve-btn-default mr-3">Skip</button>`).click(()=>this._pResolveAndClose(VeCt.SYM_UI_SKIP));
...
Charactermancer_StartingEquipment.ComponentBase = class extends BaseComponent {
    ...
    _$getBtnEnterStartingGold() {
        return $(`<button class="btn ve-btn-default btn-xs btn-5et" title="Manually enter a starting gold amount, as an alternate to rolling.">Enter Starting Gold</button>`).click(async()=>{
        ...
        const hkStartingEquipment = ()=>{
            ...
            const $rows = defaultData.map((group,ixGroup)=>{
                ...
                const $wrpsChoices = choices.map(([choiceName,choice],ixChoice)=>{
                    ...
                    const $btnSelGroup = $(`<button class="btn ve-btn-default btn-sm no-shrink ve-flex-vh-center imp-cls__disp-equi-choice-key mr-2 bold" ${isSingleOption ? "" 
...
Charactermancer_StartingEquipment.ComponentGold = class extends Charactermancer_StartingEquipment.ComponentBase {
    ...
    async _render_pGoldAlternative($wrpTabGoldAlternative) {
       ...
       /*  const {ImportListItem} = await Promise.resolve().then(function() { return ImportListItem;});
        ...
        const $btnChooseSources = this._isPredefinedItemDatas ? null : $(`<button class="btn btn-xs ve-btn-default btn-5et">Choose Item Sources</button>`).click(async()=>{
        ...
        const $btnEditPriceMultiplier = !isStandaloneGmInstance ? null : $(`<button class="btn btn-xs ve-btn-default btn-5et">Edit Config</button>`).on("click", evt=>Config.pOpen({
            evt,
        ...
        this._modalFilter.pPopulateWrapper($wrpItemList, {
            isBuildUi: true,
            $btnOpen: $(`<button class="btn-5et ve-btn-default veapp__btn-filter" name="btn-filter">Filter</button>`),
            $btnToggleSummaryHidden: $(`<button class="btn ve-btn-default btn-5et" title="Toggle Filter Summary Display" name="btn-toggle-summary"><span class="glyphicon glyphicon-resize-small"></span></button>`),
            $btnReset: $(`<button class="btn-5et ve-btn-default veapp__btn-list-reset" name="btn-reset">Reset</button>`),
        }, ).then(meta=>{
            ...
            this._renderCollection({
                ...
                fnGetNew: (itemPurchase)=>{
                 ...
                 console.log("ITEM BOUGHT", itemPurchase);
                    ...
                    const $btnSubtract = $(`<button class="btn btn-xxs btn-5et ve-btn-danger" title="Remove One (SHIFT to remove  5)"><span class="glyphicon glyphicon-minus"></span></button>`).click(evt=>{
                        if (evt.shiftKey)
        ...
        const $btnClearPurchases = $(`<button class="btn btn-xxs btn-5et ve-btn-danger" title="Remove All Purchases"><span class="glyphicon glyphicon-minus"></span></button>`).click(async()=>{
            if (!(await InputUiUtil.pGetUserBoolean({
...
class ActorCharactermancerSpell extends ActorCharactermancerBaseComponent {
    ...
    fnGetEntity: fnGetEntity
        ...
        const hkLevelChange = () => {
            const existingLevel = this._parent.compClass._getExistingClassTotalLevels();
    ...
    async _spell_pRenderFilterBox(leftContent) {
      ...
      const btnIncludeUA = ComponentUiUtil.$getBtnBool(this, "spells_isIncludeUaEtcSpellLists", {
        '$ele': $("<button class=\"btn ve-btn-default btn-xs\" title=\"Include spell lists defined in Unearthed Arcana, Plane Shift, and other semi-official products when generating the list of spells which can be learned/prepared.\">Include UA/etc. Spell Lists</button>")
      });
    ...
    _hk_onChangeClass(classIndex, {
        ...
        const cls = this._parent.compClass._getClass({ propIxClass: propIxClass });
        const subcls = this._parent.compClass._getSubclass({ cls: cls, propIxSubclass: propIxSubclass });
    ...
    _hk_onChangeAbilityScores(classIx) {
      ...
      const classInfo = this._parent.compClass._getClass({ propIxClass: propIxClass });
      const subclassInfo = this._parent.compClass._getSubclass({ cls: classInfo, propIxSubclass: propIxSubclass });
    ...
    _hk_onChangeLevel(classIx, {
        ...
        const cls = this._parent.compClass._getClass({
            'propIxClass': propIxClass
        });
        const subcls = this._parent.compClass._getSubclass({
            'cls': cls,
            'propIxSubclass': propIxSubclass
        });
...
class Charactermancer_Spell_Level extends BaseComponent {
    ...
    render($wrp) {
        ...
        const $wrpBtnsSort = $(`<div class="ve-flex-v-stretch input-group no-shrink">
			<button class="ve-btn-default btn-5et btn-xxs col-3-2 pr-1 sort" data-sort="name">Name</button>
			<button class="ve-btn-default btn-5et btn-xxs col-1-2 px-1 sort" data-sort="time">Time</button>
			<button class="ve-btn-default btn-5et btn-xxs col-1-2 px-1 sort" data-sort="school">School</button>
			<button class="ve-btn-default btn-5et btn-xxs col-0-5 px-1 sort" data-sort="concentration" title="Concentration">C.</button>
			<button class="ve-btn-default btn-5et btn-xxs col-0-5 px-1 sort" data-sort="ritual" title="Ritual">R.</button>
			<button class="ve-btn-default btn-5et btn-xxs col-2-6 px-1 sort" data-sort="range">Range</button>
			<button class="ve-btn-default btn-5et btn-xxs col-1-2 px-1 sort" data-sort="source">Source</button>
			<button class="ve-btn-default btn-5et btn-xxs col-1-6 pl-1" disabled>&nbsp;</button>
		</div>`);
    ...
    _getListItem(spell, spI) {
        ...
        eleRow.innerHTML = `
			...
			<div class="ve-flex-vh-center col-1-6 pr-0">
				<button
					class="btn ve-btn-default manc__list-row-button ${this._spellLevel === 0 ? "manc-sp__btn-learn-cantrip" : "manc-sp__btn-learn-spell"} ${this._state[ixLearned] || existingSpellMeta?.isLearned || this._state[ixAlwaysKnownSpell] ? "active" : ""}"
				...
				${this._spellLevel !== 0 ? `<button
					class="ve-btn-default btn manc__list-row-button manc-sp__btn-prepare ${this._state[ixPrepared] || this._state[ixAlwaysPrepared] || existingSpellMeta?.isPrepared || existingSpellMeta?.isAlwaysPrepared ? "active" : ""}"
					name="btn-prepare"
    ...
    async setStateFromSaveFile(actor){
        ...
        const printToState = (stateHaver, inputData) => {
            for(let [key, value] of Object.entries(inputData)){
                if(stateHaver == null){console.log("Statehaver is null!", inputData);}
        ...
        const setAdditionalSpellsStates = (featSection, statesArray) => {
            ...
            for(let ixSubComp = 0; ixSubComp < featSection._subCompsAdditionalSpells.length; ++ixSubComp){
                ...
                if(subComp == null){
                    console.error("Could not find additional spells subcomp " + ixSubComp + " for featsection", featSection);
                    continue;
                }
...
class Charactermancer_Util {
    ...
    static getFilterSearchMeta({comp, prop, propVersion=null, data, modalFilter, title}) {
        ...
        const $btnFilter = $(`<button class="btn btn-xs ve-btn-default btn-5et br-0 pr-2" title="Filter for a ${title}"><span class="glyphicon glyphicon-filter"></span> Filter</button>`).click(async()=>{
...
class Charactermancer_OtherProficiencySelect extends Charactermancer_ProficiencySelect {
    ...
    _render_renderPtChooseFromFilter($stgGroup, {ix, fromFilter, count, prop}) {
        ...
        [...new Array(count)].forEach((_,i)=>{
            ...
            const $btnFilter = $(`<button class="btn ve-btn-default btn-xxs mx-1" title="Choose a ${this.constructor._getPropDisplayName({
                prop
